// runs a js mirror of the firmware vm against the browser interpreter and asserts
// the same trace, so the two can't drift. every shape prompts/blk.md teaches compiles here.
...
assert.equal(parse("forward 99999").program[0].arg, 10000);
...
assert.equal(evalCond(parseCond("dist < 20"), null), false);
...
// ---- firmware vm ----
// a js mirror of the sketch's tickBlk(): same instruction list, same trace
function simVm(text, { sensors = {}, ai = {}, maxTicks = 4000 } = {}) {
  ...
  let pc = 0, pwm = 140, ticks = 0, guard = 0;
...
assert.equal(su.filter(l => l.startsWith("drv")).length, 2);
...
for (const src of [
  ...
  assert.doesNotThrow(() => compile(program), src);
}
